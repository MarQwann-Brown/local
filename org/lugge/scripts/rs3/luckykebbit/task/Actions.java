package org.lugge.scripts.rs3.luckykebbit.task;

import org.lugge.scripts.rs3.luckykebbit.data.Path;
import org.lugge.scripts.rs3.luckykebbit.data.Tracks;
            ...
            if (ctx.varpbits.varpbit(1218) == track.getVarpbitSetting()) {
                if (track.getNextObjectID() != 0) {
                    ...
                    if (next.inViewport()) {
                        status = "Inspect";
                        next.interact("Inspect");
                        int i = ctx.varpbits.varpbit(1218);
                        Condition.wait(new Callable<Boolean>() {
                            @Override
                            public Boolean call() throws Exception {
        ...
        for (Item item : ctx.backpack.select(new Filter<Item>() {
            ...
            if (item.id() == 36799) {
                ...
                Condition.wait(new Callable<Boolean>() {
                    @Override
                    public Boolean call() throws Exception {
                        return !ctx.backpack.select().id(item.id()).poll().valid();
                    }
                }, 500, 5);
            } else {
                item.interact("Drop");
